...
for h in range(2,2877):
    ...
    for i in range(height):
        for j in range(1, 3 * length + 1, 3):
            a = max(rgblist[j + i * (3 * length + 1)], rgblist[j + 1 + i * (3 * length + 1)],
                    rgblist[j + 2 + i * (3 * length + 1)])
            b = min(rgblist[j + i * (3 * length + 1)], rgblist[j + 1 + i * (3 * length + 1)],
                    rgblist[j + 2 + i * (3 * length + 1)])
            c = int((a + b) / 2)
        ...
        if rgblist[i * (length * 3 + 1)] == 0:
            ...
            for j in range(3 * length, 0, -1):
                if j < 4:
                    ...
                    rgblist[i * (length * 3 + 1) + j] -= (rgblist[i * (length * 3 + 1) + j - 3] + rgblist[
                        (i - 1) * (length * 3 + 1) + j]) // 2
                rgblist[i * (length * 3 + 1) + j] %= 256
    ...
    data = bytearray(typelist)
    data += bytearray(comlist)
    # print(tlonglist)
    crc = crc32(data, 0)

    crclist = list(crc.to_bytes(4,'big'))
    ...
    # print(endlist)
    testfile = open("C:\\Users\\out Man\\Desktop\\ps_png1\\{}.png".format(str(h)), mode='wb')
    testfile.write(png2)
    f.close()
    testfile.close()